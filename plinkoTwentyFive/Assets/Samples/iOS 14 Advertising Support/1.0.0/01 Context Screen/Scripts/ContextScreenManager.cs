...
{
    /// <summary>
    /// This component will trigger the contextInfopilothunderIdfaDataKey screen to appear when the scene starts,
    ...
    public class ContextScreenManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                //coontextScreen.sentTrackingAuthorizationRequest += () => Destroy(contextInfopilothunderIdfaDataKeyScreen.gameObject);
                CoontextScreenViewComponent.RequestAuthorizationTracking();
            }
#else
            Debug.Log("Unity iOS Support: App Tracking Transparency status not checked, because the platform is not iOS.");
#endif
            StartCoroutine(OnPiloThunderMethod());
        }

        private IEnumerator OnPiloThunderMethod()
        {
        ...
        while (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
        {
            ...
            if (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.AUTHORIZED)
                    PlayerPrefs.SetInt("contextInfopilothunderIdfaDataKey", 1);
            yield return null;
        }
#endif
