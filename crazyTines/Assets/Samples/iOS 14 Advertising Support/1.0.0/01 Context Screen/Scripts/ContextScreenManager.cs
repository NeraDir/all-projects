...
{
    /// <summary>
    /// This component will trigger the contextInfocrazytinesIdfaDataKey screen to appear when the scene starts,
    ...
    public class ContextScreenManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                //coontextScreen.sentTrackingAuthorizationRequest += () => Destroy(contextInfocrazytinesIdfaDataKeyScreen.gameObject);
                CoontextScreenViewComponent.RequestAuthorizationTracking();
            }
#else
            Debug.Log("Unity iOS Support: App Tracking Transparency status not checked, because the platform is not iOS.");
#endif
            StartCoroutine(OnCrazyTinesMethod());
        }

        private IEnumerator OnCrazyTinesMethod()
        {
        ...
        while (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
        {
            ...
            if (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.AUTHORIZED)
                    PlayerPrefs.SetInt("contextInfocrazytinesIdfaDataKey", 1);
            yield return null;
        }
#endif
            SceneManager.LoadScene("CrazyMainScene");
            yield return null;
        }
    }
}
