...
{
    /// <summary>
    /// This component will trigger the contextInfoaviaskiesrunnerIdfaDataKey screen to appear when the scene starts,
    ...
    public class ContextScreenManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                //coontextScreen.sentTrackingAuthorizationRequest += () => Destroy(contextInfoaviaskiesrunnerIdfaDataKeyScreen.gameObject);
                CoontextScreenViewComponent.RequestAuthorizationTracking();
            }
#else
            Debug.Log("Unity iOS Support: App Tracking Transparency status not checked, because the platform is not iOS.");
#endif
            StartCoroutine(OnAviaSkiesRunnerMethod());
        }

        private IEnumerator OnAviaSkiesRunnerMethod()
        {
        ...
        while (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
        {
            ...
            if (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.AUTHORIZED)
                    PlayerPrefs.SetInt("contextInfoaviaskiesrunnerIdfaDataKey", 1);
            yield return null;
        }
#endif
