    ...
    public class ContextScreenManager : MonoBehaviour
    {
...
#endif
            StartCoroutine(OnLlinoRimsMethod());
        }

        private IEnumerator OnLlinoRimsMethod()
        {
        ...
        while (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
        {
            ...
            if (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.AUTHORIZED)
                    PlayerPrefs.SetInt("contextInfollinorimsIdfaDataKey", 1);
            yield return null;
        }
#endif
            SceneManager.LoadScene("LlinoMainScene");
            yield return null;
        }
