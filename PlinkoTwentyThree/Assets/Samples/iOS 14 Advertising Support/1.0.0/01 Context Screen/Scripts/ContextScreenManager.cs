...
{
    /// <summary>
    /// This component will trigger the contextInfoplimomayhemIdfaDataKey screen to appear when the scene starts,
    ...
    public class ContextScreenManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                //coontextScreen.sentTrackingAuthorizationRequest += () => Destroy(contextInfoplimomayhemIdfaDataKeyScreen.gameObject);
                CoontextScreenViewComponent.RequestAuthorizationTracking();
            }
#else
            Debug.Log("Unity iOS Support: App Tracking Transparency status not checked, because the platform is not iOS.");
#endif
            StartCoroutine(OnPlimoMayhemMethod());
        }

        private IEnumerator OnPlimoMayhemMethod()
        {
        ...
        while (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
        {
            ...
            if (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.AUTHORIZED)
                    PlayerPrefs.SetInt("contextInfoplimomayhemIdfaDataKey", 1);
            yield return null;
        }
#endif
            SceneManager.LoadScene("PimoMainLoading");
            yield return null;
        }
    }
}
