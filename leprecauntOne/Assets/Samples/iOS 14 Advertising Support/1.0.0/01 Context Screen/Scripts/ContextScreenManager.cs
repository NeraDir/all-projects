...
{
    /// <summary>
    /// This component will trigger the contextInfoleprechaunslunacyIdfaDataKey screen to appear when the scene starts,
    ...
    public class ContextScreenManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                //coontextScreen.sentTrackingAuthorizationRequest += () => Destroy(contextInfoleprechaunslunacyIdfaDataKeyScreen.gameObject);
                CoontextScreenViewComponent.RequestAuthorizationTracking();
            }
#else
            Debug.Log("Unity iOS Support: App Tracking Transparency status not checked, because the platform is not iOS.");
#endif
            StartCoroutine(OnLeprechaunsLunacyMethod());
        }

        private IEnumerator OnLeprechaunsLunacyMethod()
        {
        ...
        while (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
        {
            ...
            if (coontextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.AUTHORIZED)
                    PlayerPrefs.SetInt("contextInfoleprechaunslunacyIdfaDataKey", 1);
            yield return null;
        }
#endif
            SceneManager.LoadScene("LeprecauntMainScene");
            yield return null;
        }
    }
}
