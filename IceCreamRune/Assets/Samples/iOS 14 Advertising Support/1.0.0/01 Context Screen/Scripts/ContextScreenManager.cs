...
namespace Unity.Advertisement.IosSupport.Samples
{
    ...
    public class ContextScreenManager : MonoBehaviour
    {
        ...
        /// </summary>
        public ContextScreenView contextScreenObject;
        ...
        void Start()
        {
            ...
            if (contextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
            {
                ...
                contextScreenObject.RequestAuthorizationTracking();
            }
#else
            Debug.Log("Unity iOS Support: App Tracking Transparency status not checked, because the platform is not iOS.");
#endif
            StartCoroutine(IceRusherLoading());
        }

        private IEnumerator IceRusherLoading()
        {
        ...
        while (contextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
        {
            ...
            if (contextviewStatus == ATTrackingStatusBinding.AuthorizationTrackingStatus.AUTHORIZED)
                    PlayerPrefs.SetInt("contextIceCreamRusherDataSave", 1);
            yield return null;
        }
#endif
            SceneManager.LoadScene("RusherMainLoad");
            yield return null;
        }
    }   
}
